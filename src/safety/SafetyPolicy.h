...
public:
    ...
    QString requiresApproval(const QString &toolName,
                             int filesChanged = 0,
                             int linesChanged = 0) const;

    // Configurable limits
    void setMaxIterations(int n) { m_maxIterations = n; }
    void setMaxToolCalls(int n)  { m_maxToolCalls = n; }
    void setMaxDiffLines(int n)  { m_maxDiffLines = n; }
    void setMaxChangedFiles(int n) { m_maxChangedFiles = n; }

    int maxIterations()  const { return m_maxIterations; }
    int maxToolCalls()   const { return m_maxToolCalls; }
    int maxDiffLines()   const { return m_maxDiffLines; }
    int maxChangedFiles() const { return m_maxChangedFiles; }
