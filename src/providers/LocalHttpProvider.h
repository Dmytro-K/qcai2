...
public:
    ...
    QString displayName() const override { return QStringLiteral("Local HTTP"); }
    ...
    void setEndpointPath(const QString &path) { m_endpointPath = path; }

    // If true, use simple {prompt} format instead of OpenAI chat format
    void setSimpleMode(bool simple) { m_simpleMode = simple; }

    // Custom request headers
    void setCustomHeaders(const QMap<QString, QString> &h) { m_customHeaders = h; }
