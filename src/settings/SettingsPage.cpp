...
public:
    SettingsWidget()
    {
        ...
        m_copilotModelCombo->addItems({
            ...
            QStringLiteral("claude-haiku-4.5"),
            // Google Gemini
            QStringLiteral("gemini-3.1-pro"),
            QStringLiteral("gemini-3-pro"),
            QStringLiteral("gemini-3-flash"),
            QStringLiteral("gemini-2.5-pro"),
            // xAI
            QStringLiteral("grok-code-fast-1"),
        });
        m_copilotModelCombo->setCurrentText(s.copilotModel);
        ...
        // Safety
        m_maxIterSpin  = new QSpinBox;  m_maxIterSpin->setRange(1, 50);  m_maxIterSpin->setValue(s.maxIterations);
        m_maxToolsSpin = new QSpinBox;  m_maxToolsSpin->setRange(1, 100); m_maxToolsSpin->setValue(s.maxToolCalls);
        m_maxDiffSpin  = new QSpinBox;  m_maxDiffSpin->setRange(10, 5000); m_maxDiffSpin->setValue(s.maxDiffLines);
        m_maxFilesSpin = new QSpinBox;  m_maxFilesSpin->setRange(1, 100); m_maxFilesSpin->setValue(s.maxChangedFiles);
        ...
        m_completionMinCharsSpin->setToolTip(Tr::tr("Trigger AI completion after typing this many characters"));
        ...
        m_completionModelCombo->setToolTip(Tr::tr("Leave empty to use the same model as the agent"));
