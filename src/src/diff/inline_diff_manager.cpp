...
{
    return file_path.toUrlishString().endsWith(QStringLiteral(".ui"), Qt::CaseInsensitive)
...
inline_diff_manager_t::inline_diff_manager_t(QObject *parent) : QObject(parent)
{
    if (Core::EditorManager::instance() != nullptr)
    {
        connect(Core::EditorManager::instance(), &Core::EditorManager::editorOpened, this,
                &inline_diff_manager_t::attach_widgets_for_editor);
    }
}
...
void inline_diff_manager_t::attach_hunk_widget(int index, TextEditor::BaseTextEditor *editor)
{
    ...
    if (entry.mark == nullptr || editor == nullptr || editor->editorWidget() == nullptr ||
        editor->textDocument() == nullptr || editor->textDocument()->document() == nullptr)
    {
        return;
    }

    this->track_document(editor->textDocument());
    ...
    QTextBlock block = editor->textDocument()->document()->findBlockByNumber(anchor_line - 1);
    if (!block.isValid())
    {
    ...
    if (!block.isValid())
    {
        trace_inline_diff(
            QStringLiteral("attach_hunk_widget skipped invalid block index=%1 file=%2 editor=%3 "
                ...
                .arg(document_path_string(editor->textDocument()))
                .arg(anchor_line));
        return;
    }
    ...
    if (handle)
    {
        ...
        entry.widget_handles.push_back(
            {editor, editor->textDocument(), widget, std::move(handle)});
...
void inline_diff_manager_t::attach_widgets_for_editor(Core::IEditor *editor)
{
    ...
    if (text_editor == nullptr || text_editor->textDocument() == nullptr)
    {
        return;
    }

    const Utils::FilePath editor_path = text_editor->textDocument()->filePath();
    ...
    for (std::size_t i = 0; i < this->hunks.size(); ++i)
    {
        ...
        if (hunk_path == editor_path)
        {
            ...
            this->attach_hunk_widget(int(i), text_editor);
        }
    }

    if (has_matching_hunk == true)
    {
        this->track_document(text_editor->textDocument());
    }
}
    ...
    connect(
        document, &TextEditor::TextDocument::reloadFinished, this, [this, document](bool success) {
            ...
            if (success == true)
            {
                ...
                {
                    QTimer::singleShot(0, this, [this, external_invalidation_file_path]() {
...
void inline_diff_manager_t::reattach_widgets_for_document(TextEditor::TextDocument *document)
{
    ...
    const QList<TextEditor::BaseTextEditor *> editors =
        TextEditor::BaseTextEditor::textEditorsForDocument(document);
    trace_inline_diff(
...
void inline_diff_manager_t::reattach_widgets_for_file(const QString &file_path)
{
    ...
    const QList<TextEditor::BaseTextEditor *> editors =
        TextEditor::BaseTextEditor::textEditorsForFilePath(Utils::FilePath::fromString(abs_path));
...
void flush_pending_layout_updates_for_file(const QString &project_dir, const QString &file_path)
{
    ...
    for (TextEditor::BaseTextEditor *editor : editors)
    {
        if (editor == nullptr || editor->textDocument() == nullptr ||
            editor->textDocument()->document() == nullptr)
        {
            continue;
        }

        QObject *layout = editor->textDocument()->document()->documentLayout();
        if (layout == nullptr)
        {
            continue;
        }
