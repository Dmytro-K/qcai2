                       ...
                       const QString &fieldName)
{
    ...
    for (auto it = object.begin(); it != object.end(); ++it)
    {
        if (!it.value().isString())
        {
            if (error != nullptr)
            {
                *error = QStringLiteral("Field '%1.%2' must be a string.")
...
QJsonObject serverDefinitionToJson(const ServerDefinition &definition)
{
    ...
    if (!definition.headers.isEmpty())
        object.insert(QStringLiteral("headers"), stringMapToJson(definition.headers));
    return object;
}

bool serverDefinitionFromJson(const QJsonObject &json, ServerDefinition *definition, QString *error)
{
    ...
    if (!transportValue.isUndefined() && !transportValue.isString())
    {
        if (error != nullptr)
    ...
    if (!commandValue.isUndefined() && !commandValue.isString())
    {
        if (error != nullptr)
    ...
    if (!startupTimeoutValue.isUndefined() && !startupTimeoutValue.isDouble())
    {
        if (error != nullptr)
            *error = QStringLiteral("Field 'startupTimeoutMs' must be a number.");
        return false;
    }
    if (startupTimeoutValue.isDouble())
    ...
    if (!requestTimeoutValue.isUndefined() && !requestTimeoutValue.isDouble())
    {
        if (error != nullptr)
            *error = QStringLiteral("Field 'requestTimeoutMs' must be a number.");
        return false;
    }
    if (requestTimeoutValue.isDouble())
                               ...
                               QString *error)
{
    ...
    for (auto it = json.begin(); it != json.end(); ++it)
    {
        ...
        if (!serverDefinitionFromJson(it.value().toObject(), &definition, &definitionError))
        {
            if (error != nullptr)
            {
                *error = QStringLiteral("Invalid MCP server '%1': %2")
                             .arg(it.key(), definitionError);
            }
            return false;
        }
