...
{
    return expiration.isValid() ? expiration.toString(Qt::ISODate)
                                : QStringLiteral("none");
}
...
QJsonObject oauthCacheEntryToJson(const OAuthCacheEntry &entry)
{
    ...
    if (entry.resourceMetadataUrl.isValid() && !entry.resourceMetadataUrl.isEmpty())
    {
        object.insert(QStringLiteral("resourceMetadataUrl"),
                      entry.resourceMetadataUrl.toString());
    }
    return object;
}
...
bool oauthCacheEntryFromJson(const QJsonObject &object, OAuthCacheEntry *entry)
{
    ...
    parsed.expiration = QDateTime::fromString(object.value(QStringLiteral("expiration")).toString(),
                                              Qt::ISODate);
    parsed.authorizationUrl =
        QUrl(object.value(QStringLiteral("authorizationUrl")).toString());
                                   ...
                                   QString *error = nullptr)
{
    ...
    if (existingFile.exists())
    {
        ...
        QJsonParseError parseError;
        const QJsonDocument document = QJsonDocument::fromJson(existingFile.readAll(), &parseError);
        if (parseError.error != QJsonParseError::NoError || !document.isObject())
        {
            if (error != nullptr)
            {
...
QMap<QString, QString> parseBearerChallengeParameters(const QByteArray &wwwAuthenticateHeader)
{
    ...
    const qsizetype bearerIndex = header.indexOf(
        QRegularExpression(QStringLiteral("(^|\\s|,)Bearer\\s+"),
...
{
    return appendPath(authorizationBaseUrl(endpoint), QStringLiteral("/.well-known/oauth-protected-resource"));
}

bool loadProtectedResourceMetadata(QNetworkAccessManager *networkAccessManager,
                                   const HttpTransportConfig &config,
                                   const QUrl &metadataUrl,
                                   ProtectedResourceMetadata *metadata,
                                   QString *logMessage)
{
    if (!metadataUrl.isValid() || metadataUrl.isEmpty())
        return false;
    ...
    const QJsonArray authorizationServers = root.value(QStringLiteral("authorization_servers")).toArray();
...
bool discoverOAuthEndpoints(QNetworkAccessManager *networkAccessManager,
                            ...
                            QString *logMessage)
{
    const QUrl resolvedMetadataUrl = metadataUrl.isValid() && !metadataUrl.isEmpty()
        ? metadataUrl
    ...
    if (logMessage != nullptr)
    {
        *logMessage = QStringLiteral("OAuth metadata unavailable at %1, using default authorization/token endpoint paths without assuming dynamic client registration.")
                          .arg(resolvedMetadataUrl.toString());
    }
    return true;
}
...
bool registerOAuthClient(QNetworkAccessManager *networkAccessManager,
                         ...
                         const QString &callbackUrl, RegisteredOAuthClient *client,
                         QString *error)
{
    ...
    const QJsonObject body{
        ...
        {QStringLiteral("grant_types"), QJsonArray{QStringLiteral("authorization_code"),
    ...
    const SyncHttpResponse response = performSyncRequest(
        networkAccessManager, request, QByteArrayLiteral("POST"),
...
bool startOAuthCallbackListener(QOAuthHttpServerReplyHandler *replyHandler, QString *details)
{
    ...
    if (details != nullptr)
    {
        *details = QStringLiteral(
                       "Failed to start local OAuth callback listener. Tried %1.")
...
{
    emit logMessage(QStringLiteral("Manual OAuth authorization requested: accessToken=%1 refreshToken=%2 expires=%3")
    ...
    if (!authorized && m_lastOAuthError.isEmpty())
    {
        m_lastOAuthError =
...
void HttpTransport::start()
{
    ...
    if (!m_config.endpoint.isValid() || m_config.endpoint.isEmpty())
    {
    ...
    emit logMessage(QStringLiteral("HTTP transport ready: endpoint=%1 oauth=%2")
                        .arg(m_config.endpoint.toString(),
                             m_config.oauthEnabled ? QStringLiteral("enabled")
                                                    : QStringLiteral("disabled")));
    if (m_config.oauthEnabled)
    {
        emit logMessage(QStringLiteral("Initial OAuth state: accessToken=%1 refreshToken=%2 expires=%3 clientIdConfigured=%4")
...
void HttpTransport::stop()
{
    ...
    emit logMessage(QStringLiteral("Stopping HTTP transport: %1 active request(s)")
                        .arg(m_replyStates.size()));
...
bool HttpTransport::sendMessage(const QJsonObject &message)
{
    if (m_state != State::Connected)
    {
        emit errorOccurred(QStringLiteral("Cannot send an MCP HTTP message while the transport is disconnected."));
        return false;
    }
    ...
    if (m_config.oauthEnabled)
    {
        emit logMessage(QStringLiteral("Preparing HTTP MCP request: accessToken=%1 refreshToken=%2 expires=%3")
                            .arg(tokenPresenceSummary(m_accessToken),
                                 tokenPresenceSummary(m_refreshToken),
                                 expirySummary(m_accessTokenExpiration)));
    }

    if (m_config.oauthEnabled && !m_refreshToken.isEmpty() && m_accessTokenExpiration.isValid() &&
        QDateTime::currentDateTimeUtc() >= m_accessTokenExpiration.addSecs(-30))
    {
        emit logMessage(QStringLiteral("OAuth access token is near expiry; attempting refresh before request."));
        refreshAccessToken();
    }
...
void HttpTransport::applyConfiguredHeaders(QNetworkRequest &request) const
{
    ...
    {
        if (m_config.oauthEnabled && it.key().compare(QStringLiteral("Authorization"), Qt::CaseInsensitive) == 0)
            continue;
...
{
    if (m_config.oauthEnabled && !m_accessToken.isEmpty())
    {
...
bool HttpTransport::postPayload(const QByteArray &payload, bool oauthRetried)
{
    ...
    if (m_config.oauthEnabled)
    {
        emit logMessage(QStringLiteral("Posting HTTP payload: oauthRetry=%1 accessToken=%2 refreshToken=%3 sessionId=%4")
                            .arg(oauthRetried ? QStringLiteral("yes") : QStringLiteral("no"),
                                 tokenPresenceSummary(m_accessToken),
                                 tokenPresenceSummary(m_refreshToken),
...
void HttpTransport::attachReply(QNetworkReply *reply, const QByteArray &payload, bool oauthRetried)
{
    ...
    connect(reply, &QNetworkReply::readyRead, this, [this, reply]() { processReplyReadyRead(reply); });
    connect(reply, &QNetworkReply::finished, this, [this, reply]() { processReplyFinished(reply); });
    ...
    connect(reply, &QNetworkReply::sslErrors, this, [this](const QList<QSslError> &errors) {
        ...
        if (!parts.isEmpty())
            emit logMessage(QStringLiteral("HTTP SSL errors: %1").arg(parts.join(QStringLiteral("; "))));
    });
}
...
void HttpTransport::processReplyReadyRead(QNetworkReply *reply)
{
    ...
    if (contentType == QStringLiteral("text/event-stream"))
    {
        it->isSse = true;
...
void HttpTransport::processReplyFinished(QNetworkReply *reply)
{
    ...
    if (statusCode == 401 && it->emittedMessages == 0 && m_config.oauthEnabled && !it->oauthRetried)
    {
        ...
        if (!challenge.isEmpty())
        {
            ...
            if (!resourceMetadata.isEmpty())
            {
                ...
                emit logMessage(QStringLiteral("OAuth challenge advertised protected-resource metadata: %1")
            ...
            {
                emit logMessage(QStringLiteral("OAuth challenge error description: %1")
                                    .arg(errorDescription));
            }
        }
        else if (!it->buffer.trimmed().isEmpty())
        {
    ...
    if (statusCode == 404 && !m_sessionId.isEmpty())
    {
...
void HttpTransport::processSseBuffer(QNetworkReply *reply, bool flush)
{
    while (true)
    {
        ...
        if (line.startsWith("data:"))
        {
            ...
            if (data.startsWith(' '))
                data.remove(0, 1);
            if (!it->currentEventData.isEmpty())
    ...
    if (flush && it != m_replyStates.end()
        && (!it->currentEventData.isEmpty() || !it->currentEventType.isEmpty()))
    {
        handleSseEvent(reply);
    }
}
                                     ...
                                     const QString &source)
{
    ...
    if (document.isArray())
    {
        ...
        for (const QJsonValue &messageValue : messages)
        {
            ...
            {
                emit errorOccurred(QStringLiteral("Received a non-object JSON-RPC batch item from %1.")
                                       .arg(source));
                continue;
            }
            ++it->emittedMessages;
    ...
    emit errorOccurred(QStringLiteral("Received an unsupported JSON payload from %1.").arg(source));
}
...
bool HttpTransport::ensureAuthorized()
{
    ...
    m_lastOAuthError.clear();
    emit logMessage(QStringLiteral("Ensuring OAuth authorization: accessToken=%1 refreshToken=%2 expires=%3 authorizationUrl=%4 tokenUrl=%5")
                        .arg(tokenPresenceSummary(m_accessToken),
                             tokenPresenceSummary(m_refreshToken),
                             expirySummary(m_accessTokenExpiration),
                             m_authorizationUrl.isEmpty() ? QStringLiteral("(unset)")
                                                          : m_authorizationUrl.toString(),
                             m_tokenUrl.isEmpty() ? QStringLiteral("(unset)")
                                                  : m_tokenUrl.toString()));
    ...
    if (!m_refreshToken.isEmpty())
    {
        ...
        if (refreshAccessToken())
            return true;
        emit logMessage(QStringLiteral("OAuth refresh failed; falling back to interactive authorization code flow."));
    }

    if (!m_config.interactiveOAuthEnabled)
    {
        if (m_lastOAuthError.isEmpty())
        {
            m_lastOAuthError = m_lastAuthorizationRequired
                ? QStringLiteral("OAuth authorization required.")
                : QStringLiteral("Interactive OAuth authorization is disabled.");
        }
        emit logMessage(QStringLiteral("Interactive OAuth authorization is disabled for this flow."));
        return false;
    }
...
bool HttpTransport::refreshAccessToken()
{
    ...
    {
        m_lastOAuthError = QStringLiteral("OAuth refresh cannot start because client registration, token URL, or refresh token is missing.");
        return false;
    }

    emit logMessage(QStringLiteral("Starting OAuth refresh: clientIdConfigured=%1 refreshToken=%2 tokenUrl=%3")
                        .arg(m_registeredClientId.isEmpty() ? QStringLiteral("no")
                                                            : QStringLiteral("yes"),
                             tokenPresenceSummary(m_refreshToken),
                             m_tokenUrl.toString()));
    QOAuth2AuthorizationCodeFlow oauth(m_networkAccessManager);
    oauth.setClientIdentifier(m_registeredClientId);
    if (!m_registeredClientSecret.isEmpty())
    ...
    if (!m_config.oauthScopes.isEmpty())
    ...
    QObject::connect(&oauth, &QAbstractOAuth::tokenChanged, &loop, [&](const QString &token) {
        emit logMessage(QStringLiteral("OAuth refresh tokenChanged: %1")
    ...
    QObject::connect(&oauth, &QAbstractOAuth::granted, &loop, [&]() {
        emit logMessage(QStringLiteral("OAuth refresh granted signal: token=%1 refreshToken=%2 expires=%3")
    ...
    QObject::connect(&oauth, &QAbstractOAuth::requestFailed, &loop,
                     [&](QAbstractOAuth::Error error) {
                         failure = QStringLiteral("OAuth refresh request failed: %1")
    ...
    QObject::connect(&oauth, &QAbstractOAuth2::serverReportedErrorOccurred, &loop,
                     [&](const QString &error, const QString &description, const QUrl &) {
                         failure = QStringLiteral("OAuth refresh server error: %1 (%2)")
    ...
    if (!success)
    {
        m_lastOAuthError = failure.isEmpty() ? QStringLiteral("OAuth refresh timed out.") : failure;
    ...
    emit logMessage(QStringLiteral("OAuth refresh result: accessToken=%1 refreshToken=%2 expires=%3")
                        .arg(tokenPresenceSummary(m_accessToken),
                             tokenPresenceSummary(m_refreshToken),
                             expirySummary(m_accessTokenExpiration)));
...
bool HttpTransport::runAuthorizationCodeGrant()
{
    ...
    replyHandler.setCallbackText(QStringLiteral("Authorization completed. You can return to Qt Creator."));
    QString listenDetails;
    if (!startOAuthCallbackListener(&replyHandler, &listenDetails))
    {
    ...
    if (!listenDetails.isEmpty())
        emit logMessage(listenDetails);
    emit logMessage(QStringLiteral("OAuth callback configuration: callback=%1")
                        .arg(replyHandler.callback()));
    ...
    if (!protectedResourceLog.isEmpty())
    ...
    emit logMessage(QStringLiteral("Resolved OAuth endpoints: authorization=%1 token=%2 registration=%3 resource=%4")
                        .arg(m_authorizationUrl.toString(), m_tokenUrl.toString(),
                             ...
                             m_oauthResource.isEmpty() ? QStringLiteral("(unset)")
                                                       : m_oauthResource));

    if (!m_config.oauthClientId.isEmpty())
        m_registeredClientId = m_config.oauthClientId;
    if (!m_config.oauthClientSecret.isEmpty())
        m_registeredClientSecret = m_config.oauthClientSecret;

    if (m_registeredClientId.isEmpty())
    {
        if (!endpoints.registrationEndpoint.isValid())
        {
            m_lastOAuthError = QStringLiteral("OAuth server does not expose dynamic client registration and no oauthClientId was configured.");
            emit errorOccurred(m_lastOAuthError);
            return false;
        }
        ...
        if (!registerOAuthClient(m_networkAccessManager, m_config, endpoints.registrationEndpoint,
                                 replyHandler.callback(), &registeredClient, &registrationError))
        {
    ...
    QObject::connect(&oauth, &QAbstractOAuth::authorizeWithBrowser, this, [this](const QUrl &url) {
        emit logMessage(QStringLiteral("Opening browser for OAuth authorization: %1")
                            .arg(url.toString()));
        if (!QDesktopServices::openUrl(url))
        {
            emit logMessage(QStringLiteral("Failed to open browser automatically. Open this URL manually: %1")
                                .arg(url.toString()));
        }
    });
    QObject::connect(&oauth, &QAbstractOAuth::tokenChanged, &loop, [&](const QString &token) {
    ...
    QObject::connect(&oauth, &QAbstractOAuth::granted, &loop, [&]() {
        emit logMessage(QStringLiteral("OAuth authorization granted signal: token=%1 refreshToken=%2 expires=%3")
                            .arg(tokenPresenceSummary(oauth.token()),
                                 tokenPresenceSummary(oauth.refreshToken()),
                                 expirySummary(oauth.expirationAt())));
        success = true;
    ...
    QObject::connect(&oauth, &QAbstractOAuth2::serverReportedErrorOccurred, &loop,
                     [&](const QString &error, const QString &description, const QUrl &) {
                         failure = QStringLiteral("OAuth server error: %1 (%2)")
                                       .arg(error, description);
                         loop.quit();
                     });
    QObject::connect(&timer, &QTimer::timeout, &loop, &QEventLoop::quit);
    ...
    emit logMessage(QStringLiteral("Starting OAuth authorization code flow: clientIdConfigured=%1 clientSecretConfigured=%2 scopes=%3")
                        .arg(m_registeredClientId.isEmpty() ? QStringLiteral("no")
                                                            : QStringLiteral("yes"),
                             m_registeredClientSecret.isEmpty() ? QStringLiteral("no")
                                                                : QStringLiteral("yes"),
    ...
    emit logMessage(QStringLiteral("OAuth authorization result: accessToken=%1 refreshToken=%2 expires=%3")
                        .arg(tokenPresenceSummary(m_accessToken),
                             tokenPresenceSummary(m_refreshToken),
                             expirySummary(m_accessTokenExpiration)));
    saveCachedOAuthState();
...
{
    Q_UNUSED(stage);
    if (parameters == nullptr || m_oauthResource.trimmed().isEmpty())
        return;
...
void HttpTransport::loadCachedOAuthState()
{
    ...
    emit logMessage(QStringLiteral("Looking up OAuth cache: endpoint=%1 clientIdConfigured=%2 scopes=%3")
                        .arg(m_config.endpoint.toString(),
                             m_config.oauthClientId.isEmpty() ? QStringLiteral("no")
                                                              : QStringLiteral("yes"),
                             m_config.oauthScopes.join(QStringLiteral(","))));
    ...
    if (it == oauthCache().cend())
    {
        ...
        if (loadPersistentOAuthCacheEntry(cacheKey, &persistedEntry, &persistentError))
        {
        ...
        {
            if (!persistentError.isEmpty())
            {
                emit logMessage(QStringLiteral("Persistent OAuth cache read failed: %1")
                                    .arg(persistentError));
            }
            if (!m_config.oauthClientId.isEmpty())
                m_registeredClientId = m_config.oauthClientId;
    ...
    m_registeredClientId = m_config.oauthClientId.isEmpty() ? it->clientId : m_config.oauthClientId;
    ...
    emit logMessage(QStringLiteral("Loaded cached OAuth state for %1: accessToken=%2 refreshToken=%3 expires=%4")
...
void HttpTransport::saveCachedOAuthState() const
{
    ...
    emit const_cast<HttpTransport *>(this)->logMessage(
        QStringLiteral("Saved OAuth state to cache for %1: accessToken=%2 refreshToken=%3 expires=%4")
            .arg(m_config.endpoint.toString(), tokenPresenceSummary(m_accessToken),
